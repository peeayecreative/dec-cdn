...
// new calendar code for time hide
if (str === 'on') {
  function processTimeData() {
      const Z = [];
      const H = [];
      ...
      // Hiding the time slots that fall within the specified range
      jQuery('.fc-axis.fc-time.fc-widget-content span').each(function () {
          const time = $(this).text();
          if (isTimeInRange(time, startTime, endTime)) {
              H.push(time);
              $(this).hide();
              $(this).parent().parent().hide(); // Hide the row with time
          }
      });
  ...
  function alignEvents() {
    ...
    function convertTo24Hour(time) {
      ...
      return (hours < 10 ? '0' : '') + hours + ':' + minutes;  // Format as "HH:mm"
    }
  
    // Loop through each event in Model Two (events with start and end times)
    jQuery('.fc-time-grid-event').each(function () {
      var modelOne = jQuery(this); // Current event
  
      // Get the time from the current event in Model Two
      var timeRange = modelOne.find('.fc-calendar-time').text();  // Example: "4:00 pm-4:20 pm"
      var startTime = timeRange.split('-')[0].trim();  // Extract the start time: "4:00 pm"
      var endTime = timeRange.split('-')[1].trim();    // Extract the end time: "4:20 pm"
  
      // Convert the start time and end time to 24-hour format
      var startTime24hr = convertTo24Hour(startTime);  // Example: "16:00"
      var endTime24hr = convertTo24Hour(endTime);  // Example: "16:20"
  
      // Find the corresponding row in Model One that matches the start time
      var targetRow = jQuery('tr[data-time^="' + startTime24hr + '"]');
      var endRow = jQuery('tr[data-time^="' + endTime24hr + '"]');
  
      // Calculate time slot heights and ensure both target and end rows are found
      if (targetRow.length > 0) {
        var slotHeight = targetRow.height();  // Get the height of one time slot
        var slotTopPosition = targetRow.position().top;  // Get the top position of the matched time slot
        
        // Calculate duration based on difference between start and end times
        var startTimeMinutes = parseInt(startTime24hr.split(':')[0]) * 60 + parseInt(startTime24hr.split(':')[1]);
        var endTimeMinutes = parseInt(endTime24hr.split(':')[0]) * 60 + parseInt(endTime24hr.split(':')[1]);
        var durationInMinutes = endTimeMinutes - startTimeMinutes;
        var duration = (slotHeight * durationInMinutes) / 60;  // Adjust height based on duration
  
        // Move Model One to the correct position based on the start time
        modelOne.css({
          'position': 'absolute',
          'z-index': '2',
          'top': slotTopPosition + 'px',   // Align vertically with the start time
          'left': targetRow.position().left + 'px',  // Align horizontally if necessary
          'width': '100%',  // Adjust width to match your layout needs
          'height': duration + 'px'  // Adjust height according to the duration
        });
      } else {
        console.log("Could not find row for start time: " + startTime24hr);
      }
    });
  }
  function handleButtonClick() {
  ...
  $('.fc-timeGridWeek-button.fc-button.fc-button-primary').on('click', function () {
    handleButtonClick();
    delayedAlignEvents();  // Delay alignEvents
  });
  ...
  $('button.fc-prev-button.fc-button.fc-button-primary.ecs_next_class').on('click', function () {
    delayedAlignEvents();  // Delay alignEvents
  });
  ...
  // Check on page load if the timeGridWeek view is active, and run with a delay
  if ($('.fc-view.fc-timeGridWeek-view.fc-timeGrid-view').length) {
    processTimeData();
    delayedAlignEvents();  // Delay alignEvents on page load
  }
}
// new calendar code for time hide
